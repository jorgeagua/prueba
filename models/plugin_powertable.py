...
#
# Web2py plugin powerTable ( Version ALPHA 0 ) 
...
def plugin_datatable_include():
    ...
    extrajs =  database = powerTable.get('extrajs',[])
    ui = database = powerTable.dtfeatures.get('bJQueryUI',True)
    theme = database = powerTable.get('uitheme','smoothness')
    if 'tooltip' in extrajs:
...
#iCookieDuration:400
# Extra JS Features
powerTable.extrajs = powerTable.get('extrajs',{})

powerTable.extrajs.get('editable',{})['editablecallback'] = powerTable.extrajs.get('editable',{}).get('editablecallback','../plugin_powertable/editable.load') 

powerTable.extrajs.get('details',{})['detailscallback'] = powerTable.extrajs.get('details',{}).get('detailscallback','../plugin_powertable/details')
powerTable.extrajs.get('details',{})['detailscolumns'] = powerTable.extrajs.get('details',{}).get('detailscolumns','')
...
def plugin_powertable():
    ...
    extrajs = powerTable.get('extrajs',{})
    editablecallback = extrajs.get('editable',{}).get('editablecallback','../plugin_powertable/editable.load') 
    detailscallback = extrajs.get('details',{}).get('detailscallback','../plugin_powertable/details')
    detailscolumns = extrajs.get('details',{}).get('detailscolumns','')
	...
	$('.%(_class)s tbody tr').live('click', function () {
